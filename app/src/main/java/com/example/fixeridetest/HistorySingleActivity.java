...
import com.example.fixeridetest.MPESA.PaymentMechanic;
...
public class HistorySingleActivity extends AppCompatActivity implements OnMapReadyCallback{
        ...
        historyRideInfoDb.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists() && dataSnapshot.getChildrenCount() >0){
                    ...
                    if(driverMap.get("service").equals("Battery ,Breaks or Engine Heat")){
                        ridePrice = Double.valueOf(distance) * 0.001 + 500;
                    }
                    if(driverMap.get("service").equals("Tyre Burst")){
                        ridePrice = Double.valueOf(distance) * 0.001 + 700;
                    }
                    if(driverMap.get("service").equals("Towing")){
                        ridePrice = Double.valueOf(distance) * 0.001 + 2500;
                    }
                }
            }
            @Override
    ...
    private void displayDriverRelatedObjects() {
        mPay1.setVisibility(View.VISIBLE);

        if(customerPaid){
            mPay1.setEnabled(false);
            mPay1.setVisibility(View.GONE);
        }else{
            mPay1.setEnabled(true);
    ...
    private void MpesaPayment() {
        Intent pay = new Intent(HistorySingleActivity.this, PaymentMechanic.class);
        startActivity(pay);
    }
    ...
    private void payPalPayment() {
        if (ridePrice != null) {
            ...
            // Handle the case where ridePrice is null
            Toast.makeText(this, "Service price is not available", Toast.LENGTH_LONG).show();
        }
    }
                ...
                if(dataSnapshot.exists()){
                    ...
                    if(map.get("profileimagesUrl") != null)
                    {
                        Glide.with(getApplication())
                                .load(map.get("profileimagesUrl").toString())
                                .into(userImage);
